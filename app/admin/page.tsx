...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'donations' | 'pickups' | 'users' | 'ngos'>('overview')
  ...
  const fetchData = async () => {
    setLoading(true)
    await Promise.all([fetchDonations(), fetchUsers(), fetchNgos()])
    setLoading(false)
  }
      ...
      <div className="grid md:grid-cols-7 gap-4">
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">Total NGOs</p>
  ...
  const renderNgos = () => (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Manage registered NGOs and their information
        </CardDescription>
      </CardHeader>
      <CardContent>
        {ngos.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-muted-foreground mt-2">
              NGOs can register through the admin signup process
            </p>
          </div>
        ) : (
          <div className="overflow-x-auto">
                  ...
                  <TableRow key={ngo.id}>
                    <TableCell>
                      <div>
                        ...
                        {ngo.description && (
                          <div className="text-xs text-muted-foreground max-w-xs truncate mt-1" title={ngo.description}>
                            {ngo.description}
                          </div>
                        )}
                      </div>
                    ...
                    <TableCell>
                      <div>
                        <div className="font-medium text-sm">{ngo.admin.name}</div>
                        <div className="text-xs text-muted-foreground">{ngo.admin.email}</div>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-1">
                        <MapPin className="h-3 w-3 text-muted-foreground" />
                        <span className="text-sm">{ngo.city}</span>
                      </div>
                      {ngo.address && (
                        <div className="text-xs text-muted-foreground max-w-xs truncate" title={ngo.address}>
                          {ngo.address}
                        </div>
                      )}
                    </TableCell>
                    <TableCell>
                      <div className="space-y-1">
                        {ngo.phone && (
                          <div className="text-sm">{ngo.phone}</div>
                        )}
                        {ngo.website && (
                          <a 
                            href={ngo.website} 
                    ...
                    <TableCell>
                      <div className="space-y-1">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-sm">{ngo._count.donations} donations</span>
                        </div>
                    ...
                    <TableCell>
                      <div className="text-sm">{formatDate(ngo.createdAt)}</div>
                    </TableCell>
                    <TableCell>
                      <div className="flex flex-col gap-2">
                        <Button
                          ...
                          variant="outline"
                          onClick={() => {
                            // Open map view for NGO location
                            window.open(`https://maps.google.com/?q=${ngo.latitude},${ngo.longitude}`, '_blank')
                          }}
                        >
        ...
        <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h4 className="font-medium text-blue-900">NGO Registration</h4>
              <p className="text-sm text-blue-700 mt-1">
                New NGOs can register through the admin signup process. Each NGO gets an admin account to manage their donations and profile.
              </p>
              <Button
                size="sm"
                className="mt-3 bg-blue-600 hover:bg-blue-700"
                onClick={() => window.open('/admin-signup', '_blank')}
              >
                Open NGO Registration
              </Button>
            </div>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">GiveBack Hub Management</p>
              </div>
            </div>
            <div className="flex items-center space-x-4">
              <div className="text-right">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-gray-900">{session?.user?.name || 'Admin'}</span>
                </div>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">Manage users, donations, NGOs, and track geographic impact</p>
        </div>
